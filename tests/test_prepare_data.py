...
def read_and_diff(ds_reader, path, test_data, measures, dimensions,
                  continuous_obs, basis):
    dataset = dict(id='')
    fs = fsspec.filesystem('file')
    prepared_adata = ds_reader.read_dataset(filesystem=fs, path=path,
                                            dataset=dataset,
                                            keys=dict(X=measures,
                                                      obs=dimensions + continuous_obs,
                                                      basis=[basis]))

    assert scipy.sparse.issparse(test_data.X) == scipy.sparse.issparse(
        prepared_adata.X)
    ...
    for key in dimensions + continuous_obs:
        pd.testing.assert_series_equal(test_data.obs[key],
    ...
    prepared_adata2 = ds_reader.read_dataset(filesystem=fs, path=path,
                                             dataset=dataset, keys=dict())
    assert prepared_adata2.shape[0] == test_data.shape[0]


def test_prepare_cxg_tile_db(test_data, measures, dimensions, continuous_obs,
                             basis, tmp_path):
    try:
        ...
        subprocess.check_call(['cellxgene', 'convert', '-o', output_cxg,
                               '--disable-corpora-schema', output_h5ad])
        read_and_diff(TileDBDataset(), output_cxg, test_data, measures,
...
def test_prepare(test_data, measures, dimensions, continuous_obs, basis,
    ...
    test_data.obs = test_data.obs[dimensions + continuous_obs]
    if spatial:
        test_data.uns['images'] = [dict(type='image', name='tissue_hires',
                                        image=output_dir)]
...
def test_prepare_jsonl(test_data, measures, dimensions, continuous_obs, basis,
                       tmp_path):
    output_dir = str(tmp_path)
    test_data = test_data[:, measures]
    test_data.obs = test_data.obs[dimensions + continuous_obs]
