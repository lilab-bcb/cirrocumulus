...
The `launch` command opens one or more datasets in h5ad_, loom_, or `STAR-Fusion`_ format. Seurat objects
...
The server can be deployed on a cloud VM, an on-premise machine, or on Google App Engine. Note that when deployed in App Engine, the server can be configured
to be used Google Cloud Firestore as a database. Please note that when you start a cirrocumulus server, no datasets are available until
