...
export class ScatterPlotVisualizerSprites implements ScatterPlotVisualizer {
  ...
  onPointPositionsChanged(newPositions: Float32Array) {
    ...
    const positions = (this.points.geometry as BufferGeometry).getAttribute(
      "position"
    ) as BufferAttribute;
  ...
  onRender(rc: RenderContext) {
    ...
    this.renderMaterial.uniforms.zoomFactorSpecified.value = !isNaN(
      this.zoomFactor
    );
    ...
    let scaleFactors = (this.points.geometry as BufferGeometry).getAttribute(
      "scaleFactor"
    ) as BufferAttribute;
