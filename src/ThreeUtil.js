...
export function getPositions(trace) {
  ...
  if (!is3d && trace.ranks == null) {
    trace.ranks = !trace.isCategorical
      ? rankIndexArray(indexSort(trace.values, true))
      : randomSeq(trace.values.length, 1);
    // ranks go from 1 to values.length. Higher rank means higher value.
    const zNormScale = scaleLinear()
      .domain(extent(trace.ranks))
      .range(Z_RANGE_2D);
    zExtent = Z_RANGE_2D;
  ...
  const maxXYZRange = is3d
    ? Math.max(xRange, yRange, zRange)
    : Math.max(xRange, yRange);
  const halfCube = SCATTER_PLOT_CUBE_LENGTH / 2;
  const makeScaleRange = (range, base, maxRange) => [
    -base * (range / maxRange),
    base * (range / maxRange),
  ];
  ...
  const zScale = is3d
    ? makeScaleRange(zRange, halfCube, maxXYZRange)
    : makeScaleRange(zRange, halfCube, zRange);
...
export function updateScatterChart(
  ...
  scatterPlot.scene.background = chartOptions.darkMode
    ? new Color('rgb(0, 0, 0)')
    : null;
  ...
  if (labelsVisualizer) {
    ...
    if (showLabels) {
      const labelKey =
        getEmbeddingKey(trace.embedding) + '_' + obsCatKeys.join(',');
      ...
      labelsVisualizer.shadowColor = chartOptions.darkMode
        ? 'rgba(0,0,0,0.9)'
        : 'rgba(255,255,255,0.9)';
      labelsVisualizer.shadowStroke = chartOptions.labelStrokeWidth;
      labelsVisualizer.font =
