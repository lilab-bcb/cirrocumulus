...
export const DATASET_FIELDS = [
  ...
  {fieldName: 'overallDesign', label: 'Overall Design'},
  {fieldName: 'citations', label: 'Citation(s)'},
];
...
function EditNewDatasetDialog(props) {
  ...
  const setFileDropRef = useCallback(
    (node) => {
  ...
  useEffect(() => {
    ...
    function toArray(key) {
      return dataset != null && dataset[key] != null && dataset[key] != ''
        ? isArray(dataset[key])
          ? dataset[key]
          : [dataset[key]]
        : [];
    ...
    return value.map((option, index) => {
      ...
      return (
        <Chip
          ...
          sx={{
            textDecoration:
              atIndex > 0 && option.lastIndexOf('.') > atIndex
                ? null
                : 'line-through',
          }}
          {...getTagProps({index})}
  ...
  return (
    <Dialog
      ...
      maxWidth={'lg'}
    >
      <DialogTitle id="edit-dataset-dialog-title">
        {isNew ? 'New' : 'Edit'} Dataset
      </DialogTitle>
      <DialogContent>
        <Box>
          <TextField
          ...
          <div style={{display: isNew && canUpload ? '' : 'none'}}>
            ...
            <div role="tabpanel" hidden={uploadTabValue !== 'My Computer'}>
              ...
              <TextField
                ...
                helperText={
                  serverInfo.email
                    ? 'Please ensure that ' +
                      serverInfo.email +
                      ' has read permission to this URL'
                    : ''
                }
                label={'URL'}
                fullWidth
          ...
          <FormControl className={classes.formControl}>
            ...
            <Select
              ...
              value={species}
              onChange={(event) =>
                setSpecies(event.target.value.filter((item) => item !== ''))
              }
            >
          ...
          <FormControl className={classes.formControl}>
            ...
            <FormHelperText style={{marginTop: 14}}>
              <Link
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                ...
                size={'small'}
                label="Contact(s)"
                helperText={'Enter list of contact emails'}
              />
            )}
          />
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                ...
                helperText={
                  isAuthEnabled
                    ? 'Enter list of emails for sharing'
                    : 'Enable authentication to permit sharing'
                }
              />
            )}
          />
