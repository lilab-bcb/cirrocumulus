...
import {
  sortAndFilterJobResult,
  updateJob,
  updateTopNJobResult,
} from './DotPlotJobResultsPanel';
import {
  createColorScale,
...
class DotPlotJobResultOptions extends React.PureComponent {
  ...
  onInterpolator = (value) => {
    ...
    jobResult.colorScale = createColorScale(jobResult.interpolator)
      .domain(jobResult.colorScale.domain())
      .clamp(true);
    this.handleJobResults();
  };
  ...
  render() {
    const {jobResult, classes, textColor} = this.props;
    ...
    return (
      <div className={classes.leftMargin}>
        <div>
          <Typography component={'h2'}>Rank Features</Typography>
          <FormControl
            style={{display: 'block'}}
            ...
            >
              ...
              >
                {jobResult.columns.map((index) => (
                  <MenuItem
                    key={jobResult.groups[index]}
                    value={jobResult.groups[index]}
                  >
                    {jobResult.groups[index]}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
        ...
        <Tooltip title="Filters are applied separately per cluster">
        ...
        {rowFilters.map((filter) => {
          ...
          return (
            <div key={filter[0]} style={{paddingTop: 8}}>
              ...
              <FormControl>
                <Select
                  ...
                  value={filter[1]}
                  onChange={(event) =>
                    this.onOperationChanged(filter, event.target.value)
                  }
                >
              ...
              <TextField
                ...
                autoComplete={'off'}
                onChange={(event) =>
                  this.onValueChange(filter, event.target.value)
                }
                value={filter[3]}
        ...
        <Typography style={{marginTop: 16}} component={'h2'}>
          View Options
        </Typography>
        ...
        {jobResult.groups.length > 1 && (
          <FormControl className={this.props.classes.formControl}>
            ...
            <Select
              ...
              size={'small'}
              onChange={(event) =>
                this.onColorScalingChange(event.target.value)
              }
