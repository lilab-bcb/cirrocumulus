...
export function sortAndFilterJobResult(jobResult) {
  ...
  rowFilters.forEach((filter) => {
    if (!isNaN(filter[2])) {
      rowFilterFunctions.push({
        field: filter[0],
        f: createFilterFunction(filter),
      });
    }
  });
...
function DotPlotJobResultsPanel(props) {
  ...
  useEffect(() => {
    ...
    if (
      jobResult.size !== jobResult.by &&
      isSizeScaled &&
      jobResult.size !== jobResult.color
    ) {
  ...
  const onMouseMove = useCallback((event) => {
    setTip({
      html:
        event.target.dataset.title == null ? '' : event.target.dataset.title,
  ...
  return (
    <div>
      ...
      >
        <b>{jobResult.name}</b>&nbsp;
        <small>
          {intFormat(jobResult.rows.length) +
            ' / ' +
            intFormat(jobResult.data.length) +
            ' features'}
        </small>
      </Typography>
