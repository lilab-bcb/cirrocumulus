  ...
  return (
    <div style={{position: 'relative', display: 'inline-block'}}>
      <canvas
        ...
        style={{
          width: options.violinWidth,
          height: size.y + options.violinHeight,
        }}
      />
      <CirroTooltip
        html={tip.html}
...
function drawCategory(
  ...
  if (violinScale === 'width') {
    yscale = scaleLinear()
      .domain([-item.density.max, item.density.max])
      .range([4, violinWidth - 4]); // horizontal position
  }
...
export function getViolinPlotScales(data, featureIndex, options) {
  ...
  const xscale = scaleLinear()
    .domain([xmin, xmax])
    .range([violinHeight - 1, 6])
    .nice(); // vertical position
  ...
  return (
    ...
    >
      ...
      <Axis
        xscale={xscale}
        textColor={textColor}
        violinHeight={options.violinHeight}
        size={size}
      />
