...
class BackedSparseMatrix(_cs_matrix):
    ...
    def _set_many(self, i: Iterable[int], j: Iterable[int], x):
        ...
        if -1 not in offsets:
            # make a list for interaction with h5py or zarr
...
class SparseDataset:
    """Analogous to :class:`h5py.Dataset or zarr.Group`, but for sparse matrices."""
    ...
    def __repr__(self) -> str:
        return (
            f"<sparse dataset: format {self.format_str!r}, "
            f"shape {self.shape}, "
